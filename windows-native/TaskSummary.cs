    ...
    public sealed partial class MainForm
    {
        ...
        private RoundedComposerPanel BuildTaskSummary(ChatMessage message,int width)
        {
            ...
            foreach(FileChangeSummary file in files)
            {
                ...
                open.Click+=delegate{try{if(File.Exists(file.path))System.Diagnostics.Process.Start("explorer.exe","/select,\""+file.path+"\"");}catch(Exception ex){MessageBox.Show(this,ex.Message,"Файл");}};row.Controls.Add(open);
