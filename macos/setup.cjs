...
function probe(root){
  fs.mkdirSync(root,{recursive:true,mode:0o700});let major=0,chip='Не определён',osVersion=os.release(),ramBytes=NaN;
...
async function install(root,{signal,onProgress=()=>{},engineOnly=false}={},services={}){
  const scan=services.probe||probe,fetchComponent=services.download||download,checkComponents=services.checkEngine||checkEngine,execute=services.run||run,locate=services.findEngine||findEngine;
  const verify=()=>{
