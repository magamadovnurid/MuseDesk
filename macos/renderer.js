...
function updateTopicActive(){const host=$('messages'),marks=[...$('topic-nav').children],top=host.getBoundingClientRect().top;let active=0;marks.forEach((mark,i)=>{const row=host.children[Number(mark.dataset.message)];if(row&&row.getBoundingClientRect().top-top<=24)active=i;});if(host.scrollHeight>host.clientHeight&&host.scrollTop>=host.scrollHeight-host.clientHeight-2)active=marks.length-1;marks.forEach((mark,i)=>{if(i===active)mark.setAttribute('aria-current','true');else mark.removeAttribute('aria-current');});}
function layoutTopics(){const host=$('messages'),nav=$('topic-nav'),count=nav.children.length;nav.hidden=count===0;const step=Math.min(14,Math.min(360,Math.max(1,host.clientHeight-40))/Math.max(1,count-1));[...nav.children].forEach((mark,index)=>{mark.style.top=((host.clientHeight-step*(count-1))/2+index*step)+'px';mark.style.height=Math.min(14,step)+'px';});updateTopicActive();}

function showTopicPreview(mark,title,preview,x,y){topicPreview.querySelector('strong').textContent=title;topicPreview.querySelector('p').textContent=preview||'Перейти к этому вопросу';topicPreview.classList.remove('hidden');mark.setAttribute('aria-describedby','topic-preview');topicPreview.style.left=Math.max(8,Math.min(innerWidth-topicPreview.offsetWidth-8,x+18))+'px';topicPreview.style.top=Math.max(8,Math.min(innerHeight-topicPreview.offsetHeight-8,y+14))+'px';}
...
const isLinux=window.muse.platform==='linux';
if(isLinux){document.body.classList.add('linux');document.querySelector('.platform').textContent='Ubuntu';document.querySelector('.setup-intro li').textContent='Проверка компьютера';const controls=make('div','window-controls');for(const [label,text,action] of [['Свернуть','−','window-minimize'],['Развернуть / восстановить','□','window-maximize'],['Закрыть','×','window-close']]){const control=button(text,()=>api(action));control.setAttribute('aria-label',label);controls.append(control);}document.getElementById('titlebar').append(controls);}
...
function renderSetupHardware(h){
  ...
  $('setup-title').textContent=eligible?'Glimmer подходит вашему компьютеру':h.profile==='unsupported'?'Эта система не поддерживается':h.profile==='unverified'?'Не удалось проверить систему':'Muse Glimmer не подходит этому компьютеру';
  $('setup-description').textContent=eligible?'Перед скачиванием параметры компьютера будут проверены повторно.':h.profile==='unsupported'?(isLinux?'Нужна Ubuntu 22.04 или новее, x86-64.':'Для этой версии приложения нужны Apple Silicon и macOS 14+.')+' Установка компонентов заблокирована.':h.profile==='unverified'?'Сначала нужно подтвердить параметры системы. Скачивание заблокировано.':'Модель не будет скачана. Приложение можно оставить для просмотра чатов; работа с Glimmer недоступна.';
  ...
  $('hardware').textContent=h.chip+' · '+(isLinux?'':'macOS ')+h.osVersion+'\n'+(isLinux?'Оперативная память: ':'Установленная объединённая память: ')+memory+'\nСвободно на диске: '+disk+(isLinux&&h.gpu?'\nGPU: '+h.gpu.name+' · '+(h.gpu.vramBytes/1024**3).toFixed(1)+' ГиБ VRAM':'');
  $('setup-profile').textContent=eligible?'Glimmer Q4_K_M + '+(h.profile.endsWith('f16')?'F16':'Q8')+' кодировщик. Загрузка около '+(h.profile.endsWith('f16')?'21':'19')+' ГБ.':(h.compatibility?.reasons||[isLinux?'Для Glimmer нужны 32 ГБ RAM, NVIDIA 24 ГБ VRAM с драйвером 550+ и 40 ГиБ для установки.':'Для Glimmer нужны Apple Silicon, macOS 14+, не менее 32 ГБ объединённой памяти и 40 ГиБ для установки.']).join(' ');
...
bind('speak',()=>{if(isLinux)return api('speak',{id:data.activeId});if(speechSynthesis.speaking){speechSynthesis.cancel();return;}const text=active()?.messages.filter(m=>m.role==='assistant').at(-1)?.content;if(text){const utterance=new SpeechSynthesisUtterance(text);utterance.lang='ru-RU';speechSynthesis.speak(utterance);}});
...
$('messages').addEventListener('scroll',()=>{const el=$('messages');followTail=el.scrollHeight-el.scrollTop-el.clientHeight<70;updateTopicActive();});
