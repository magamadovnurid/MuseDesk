...
async function openSetup(){setupHardware=await api('hardware');const h=setupHardware,eligible=h.profile.startsWith('glimmer');$('setup-title').textContent=eligible?'Glimmer подходит вашему Mac':h.profile==='unsupported'?'Этот Mac пока не поддерживается':'Начнём с приложения';$('setup-description').textContent='Модель работает непосредственно на Mac. Скачивание требует доступа к GitHub и Hugging Face.';$('hardware').textContent=h.chip+' · macOS '+h.osVersion+'\nОбъединённая память: '+Math.round(h.ramBytes/1024**3)+' ГБ\nСвободно: '+Math.floor(h.freeBytes/1024**3)+' ГБ';$('setup-profile').textContent=eligible?'Glimmer Q4_K_M + '+(h.profile.endsWith('f16')?'F16':'Q8')+' кодировщик изображений. Загрузка около '+(h.profile.endsWith('f16')?'21':'19')+' ГБ.':'Для Glimmer нужны Apple Silicon, macOS 14+, 32 ГБ памяти и 40 ГБ свободного места.';$('setup-install').disabled=h.profile==='unsupported'||data.busy;$('setup-install').textContent=eligible?'Установить Glimmer':'Установить движок';$('setup-message').textContent='';$('setup-cancel').textContent='Позже';$('setup-progress').style.width='0';$('setup-progress').classList.remove('indeterminate');if(!$('onboarding').open)$('onboarding').showModal();}
async function install(){if(data.busy)return;$('setup-install').disabled=true;$('setup-cancel').textContent='Остановить';$('setup-close').disabled=true;try{await update('install',{engineOnly:!setupHardware.profile.startsWith('glimmer')});$('setup-message').textContent='Готово. Можно закрыть это окно.';$('setup-cancel').textContent='Готово';}catch(error){$('setup-message').textContent=error.message;$('setup-install').textContent='Повторить';}finally{$('setup-close').disabled=false;$('setup-install').disabled=false;$('setup-cancel').textContent='Закрыть';}}
